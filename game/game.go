...
import (
	...
	"github.com/vhespanha/tour_clara/ui"
)
...
func New() *Game {
	return &Game{
		fonts:            ui.NewFonts(),
		dialogue:         dialogue.New(ScreenWidth, ScreenHeight),
...
func (g *Game) Draw(screen *ebiten.Image) {
	if g.isOnDeathScreen {
		ui.DrawDeathScreen(screen, g.fonts, ScreenWidth, ScreenHeight)
		return
	}
...
func createChoiceRects(box *geometry.Rect, n int, gap, padding float32) []*geometry.Rect {
	...
	if n == 1 {
		x := box.Pos.X + padding
		return []*geometry.Rect{geometry.NewRect(x, y, available, float32(h))}
	}
